...
class AIModelGUI:
    """
    Main GUI Application for AI Models.
    
    Demonstrates:
    - Encapsulation: Private methods and attributes
    - Composition: Uses AIModel instances
    """
...
7. COMPOSITION:
   ...
   - Why: "Has-a" relationship - GUI has models, doesn't inherit from them
    ...
    def __update_model_info(self):
        ...
        self.model_info_text.insert(1.0, f"{'='*50}\n")
        self.model_info_text.insert(tk.END, f"  {model_name.upper()}\n")
        self.model_info_text.insert(tk.END, f"{'='*50}\n\n")
    ...
    def __process_input(self):
        """Process input through selected model"""
        model_name = self.model_var.get()
        model = self.__models[model_name]
